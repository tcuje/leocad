...
//Redefinimos el método rotarConector de la clase conector para el Spur.
void Spur::rotarConector(sirMat4d r,int tooth/*,unsigned short nTime,bool bAnimation,bool bAddKey*/)
{
	Conector *actual;
	sirMat4d C2W, A2W;
	sirVector4d normal(0,0,1,0),vector, vector2;
	sirVector3d rote, ruta, auxili, trans;
	float rtooth,rot[4],rot2[4];
	bool rotarZ;
	...
	else
	{
		...
		if((rot[0]==vector[0])&&(rot[1]==vector[1])&&(rot[2]==vector[2]))
			rot[3]=(-1)*rot[3];
		...
		while(!conectados.Fin())
		{
			...
			if(rotarZ)
			{
				...
				if(strcmp(actual->obtenerNombre(),"Spur")==0)
					if(iguales(vector2[0],vector[0])&&iguales(vector2[1],vector[1])&&iguales(vector2[2],vector[2]))
					{
						rot2[3]=rot[3];
					}
					else
				...
				if(strcmp(actual->obtenerNombre(),"Rack")==0)
				{
					...
					/******************************/
					trans=obtenermovimiento(rot[3],vector,actual);
					actual->mover(trans);
				}
				else
...
sirVector3d Spur::obtenermovimiento(float rtooth,sirVector4d vector,Conector *actual)
{
	sirVector4d normal(0,0,1,0),x(0,1,0,0),vec,vector2,vectory;
	sirVector3d resultado,p1,p2,prodVect;
	sirMat4d C2W,D2W;
	Administrador *ad=NULL;
	double aux,aux2,normaProdVect,prodEscalar,alfa;

	rtooth=(-1)*rtooth;


	D2W=actual->obtenerC2W();
	vector2=D2W*normal;
	vectory=D2W*x;
	p1[0]=vectory[0];
	p1[1]=vectory[1];
	...
	p2.normalize();//normaEstatico=normal.normalize();
//	normaEstatico=normal.norm();
	p1.normalize();//normaDinamico=normal2.normalize();
//	normaDinamico=normal2.norm();

	//Producto vectorial-->estatico X dinamico-->VIGILAR EL ORDEN!!!
	prodVect=p2^p1; //prodVect=normal^normal2;
	...
	prodEscalar=p2*p1;//prodEscalar=normal*normal2;
	...
	//obtener la distancia utilizando el numero de dientes de la pieza spur con el vector normal
	//del rack
	aux=this->obtenerNdientes();
	aux=aux/360;

	aux=aux*rtooth;

	aux2=actual->obtenerNdientes();
	aux2=((actual->obtenerradio())*2)/actual->obtenerNdientes();

	aux=aux*aux2;

	vector2=vector2*aux;

	resultado[0]=vector2[0];
	resultado[1]=vector2[1];
	resultado[2]=vector2[2];
...
void Spur::moverConector(sirVector3d t)
{
	Conector *pCon;
...
bool Spur::rotarEjeZ(sirMat4d r)
{
...
double Spur::anguloEntreEjes(sirVector4d ejeDinamico,sirVector4d ejeEstatico,sirMat4d *m)
{
...
bool Spur::Ultimo_contacto(sirVector3d pos1,sirVector3d pos2, sirVector3d t)
{
	int i;
	bool igual=true;
	sirMat4d C2W,W2C;
	...
	if(iguales(trans[0],0) && iguales(trans[1],0) && (iguales(trans[2],1) || iguales(trans[2],-1))) return false;
	else
	{
		for(i=0;i<3 && igual==true;i++)
		{
			if(t[i]!=0)
				if(pos1[i]!=pos2[i])
					igual=false;
